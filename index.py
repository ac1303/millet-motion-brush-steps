...
class MiFit:
    ...
    def pushDeer(self,text):
        if(len(self.pushDeerKey)==40):
            print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),"开始推送数据到手机")
            ...
            if(req['content']['result']==False):
                print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),"密钥不正确")
            else:
                for list in req['content']['result']:
                    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),"设备："+list['trace_id'],"推送"+list['description'])
        else:
            print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),"PushDeer的推送密钥不正确，请重新填写")
    ...
    def _sync_data(self, login_token, user_id, step):
        ...
        # print(response.text)
        if response.status_code == 200:
            self.pushDeer("随机步数为："+str(step)+"  步数提交成功！")
            return "数据同步成功"
        else:
            self.pushDeer("随机步数为："+str(step)+"  步数提交失败！错误信息请查看日志！")
